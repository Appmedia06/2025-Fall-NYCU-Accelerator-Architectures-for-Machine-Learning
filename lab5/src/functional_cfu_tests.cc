...
void do_matmul_num(int test_num) {
  ...
  */ 
  ...
  for (int i = 0; i < 64; i++) {
      cfu_op0(1, i, A_arr[test_num][i]);    
      cfu_op0(2, i, B_arr[test_num][i]);
  }  
  ...
  for (int j = 0; j < 16; j += 4) {
    for (int i = 0; i < 16; i++) {
      C_arr[i][j    ] = cfu_op0(4, C_index, C_offset    );
      C_arr[i][j + 1] = cfu_op0(4, C_index, C_offset + 1);
      C_arr[i][j + 2] = cfu_op0(4, C_index, C_offset + 2);
      C_arr[i][j + 3] = cfu_op0(4, C_index, C_offset + 3);
      C_index++;
    }
  }  
