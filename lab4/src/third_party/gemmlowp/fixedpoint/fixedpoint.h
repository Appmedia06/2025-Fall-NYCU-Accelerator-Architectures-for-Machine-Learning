...
// Returns exp(x) for x < 0. [CHAO]
template <typename tRawType, int tIntegerBits>
FixedPoint<tRawType, 0> exp_on_negative_values(
    FixedPoint<tRawType, tIntegerBits> a) {
  
  int myIntegerBits = a.kIntegerBits;
  int32_t exp;
  // exp
  if ( myIntegerBits == 4 ) { 
    exp = cfu_op0(0, a, 0);
  ...
  // softmax
  else if ( myIntegerBits == 5 ) {
    exp = cfu_op0(2, a, 0);
  }
  
  FixedPoint<tRawType, 0> ans = FixedPoint<tRawType, 0>::FromRaw(exp);
  return ans;
}

// template <typename tRawType, int tIntegerBits>
// FixedPoint<tRawType, 0> my_exp_on_negative_values( FixedPoint<tRawType, tIntegerBits> a) {
//   int32_t result = cfu_op0(0, a, 0);
//   typedef FixedPoint<tRawType, 0> ResultF = (tRawType)result;
//   return ResultF;
// }
...
FixedPoint<tRawType, 0> one_over_one_plus_x_for_x_in_0_1(
  ...
  int32_t x = cfu_op0(1, a, 0);
  FixedPoint<tRawType, 0> ans = FixedPoint<tRawType, 0>::FromRaw(x);
  return ans;  
}
