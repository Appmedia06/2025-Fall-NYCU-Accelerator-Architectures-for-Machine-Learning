...
inline void ConvPerChannel(
  ...
  const int output_depth = 300; //MatchingDim(filter_shape, 0, output_shape, 3);
  if (bias_data) {
          ...
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              for (int in_channel = 0; in_channel < filter_input_depth; in_channel += 4) {
                ...
                // In the first convolution layer (300, 3, 3, 1), the filter_input_depth is 1. According to the given parameters, the input offset for this layer is always -102.
                if ( input_offset == -102 ) { // if it is first conv, do software
                  uint32_t input_val =  input_data[Offset(input_shape, batch, in_y, in_x, in_channel + group * filter_input_depth)];
                  uint32_t filter_val = filter_data[Offset(filter_shape, out_channel, filter_y, filter_x, in_channel)];
                  // software
                  acc += filter_val * (input_val + input_offset);
                }
                // In the remaining convolution layers (300, 1, 1, 300), the filter_input_depth is 300. According to the parameters, the input offset for these layers is always 128.
                else {
